#!/usr/bin/env python3
"""stress_test_c2_vs_baseline — backtest 3 fenêtres adversariales pour C2 vs prod.
...
Configs:
  C2   : vt=1.00 lev=64 vfl=0.02 EnableDDCap=false (variant growth)
  prod : vt=0.28 lev=12 vfl=0.02 EnableDDCap=false (pre-Phase I default)

Critères pass:
  - DD(C2) ≤ 25% (hard cap)
  - DD(C2) ≤ 2.0 * DD(prod) (relâché vu agressivité C2 vs C1=1.5)
  - Sharpe(C2) ≥ 0
"""
...
CONFIGS = {
    "C2":   {"Inp_GlobalTargetVol": "1.00", "Inp_GlobalMaxLeverage": "64",
...
def main() -> int:
    ...
    print(f"  {'Window':<20} {'DD_C2':>7} {'DD_prod':>8} "
          f"{'Ratio':>6} {'C2_PASS?':>10}")
    print("  " + "-" * 60)
    for win_name, _, _ in WINDOWS:
        c1 = df[(df["config"] == "C2") & (df["window"] == win_name)]

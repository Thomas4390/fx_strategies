"""Phase 19 — refined leverage sweep around the Phase 18 trio.

Follow-up to ``scripts/sweep_combinations.py``. Drills into the promising
region discovered in the 37-config Phase 18 sweep (E1/E5/D5/D6 on the
P18 trio `MR_Macro + TS_Momentum_3p + RSI_Daily_4p`) with a much denser
grid on ``target_vol × max_leverage × dd_cap_enabled``, plus a small
weights-variation sub-block around the ``80/10/10`` baseline.

Grid (~120 configs)
-------------------
**Core dense grid** (Phase 18 weights fixed at 80/10/10):
...
alternative P18-trio weight mixes):
  - 85/10/5, 80/15/5, 80/10/10 (dup for reference), 75/15/10, 75/10/15,
    70/20/10, 70/15/15
  - Total: **7 configs**

**Baselines** (exact Phase 18 prod and top-3 from previous sweep):
  - P18 prod (MR80/TS3p10/RSI10 tv=0.28 ml=12 DDoff)
...
Usage
-----
    .venv/bin/python scripts/sweep_phase19.py               # full run
    .venv/bin/python scripts/sweep_phase19.py --no-bootstrap
    .venv/bin/python scripts/sweep_phase19.py --smoke       # 4 combos only
"""
...
_P18_WEIGHTS = {"MR_Macro": 0.80, "TS_Momentum_3p": 0.10, "RSI_Daily_4p": 0.10}
...
def build_phase19_grid() -> list[SweepConfig]:
    """Return the full Phase 19 sweep grid (~116 configurations)."""
    ...
    for tv in TARGET_VOLS:
        for ml in MAX_LEVS:
            for dd in DD_CAPS:
                ...
                cid = f"P19c-tv{tv_tag}-ml{ml_tag}-{dd_tag}"
                cfgs.append(
                    SweepConfig(
                        id=cid,
                        block="CORE",
                        name=f"P18 weights / tv={tv:.2f} ml={int(ml)} {'DDon' if dd else 'DDoff'}",
    ...
    HLEV_GRID = [
        (0.32, 18.0), (0.32, 20.0), (0.32, 24.0),
        (0.35, 18.0), (0.35, 20.0), (0.35, 24.0),
        (0.40, 18.0), (0.40, 20.0), (0.40, 24.0),
    ]
    for tv, ml in HLEV_GRID:
        cfgs.append(
            SweepConfig(
                id=f"P19h-tv{int(tv*100):02d}-ml{int(ml):02d}",
                block="HLEV",
                name=f"P18 weights / tv={tv:.2f} ml={int(ml)} DDon (high-lev)",
                sleeves=_P18_SLEEVES,
                allocation="custom",
                target_vol=tv,
                max_leverage=ml,
    ...
    # ─── Block WEIGHTS — P18 trio weight variations (7 configs) ───
    # Applied to a plausible "good-looking" core point (tv=0.28 ml=14 DDon)
    # which is E5-like from the previous sweep. The idea is: if E5-like is
    # a sweet spot, does a different weight mix around it do better?
    WEIGHT_VARIATIONS = [
    ...
    cfgs += [
        SweepConfig("BL-P18prod", "BASELINE",
                    "Phase 18 prod (MR80/TS3p10/RSI10 tv=0.28 ml=12 DDoff)",
                    _P18_SLEEVES, "custom", 0.28, 12.0, False,
                    custom_weights=dict(_P18_WEIGHTS)),
        SweepConfig("BL-E5", "BASELINE",
                    "E5 (P18 weights / tv=0.28 ml=14 DDon)",
                    _P18_SLEEVES, "custom", 0.28, 14.0, True,
                    custom_weights=dict(_P18_WEIGHTS)),
        SweepConfig("BL-E1", "BASELINE",
                    "E1 (P18 weights / tv=0.28 ml=6 DDon)",
                    _P18_SLEEVES, "custom", 0.28, 6.0, True,
                    custom_weights=dict(_P18_WEIGHTS)),
        SweepConfig("BL-D1", "BASELINE",
                    "D1 (P18 weights / tv=0.10 ml=3 DDon)",
                    _P18_SLEEVES, "custom", 0.10, 3.0, True,
                    custom_weights=dict(_P18_WEIGHTS)),
    ]
...
def build_phase19_markdown(
    ...
    lines: list[str] = []
    lines.append(f"# Phase 19 — Refined leverage sweep ({report_date})\n")
    lines.append(
        "Suite directe du sweep Phase 18 (`docs/research/sweep_2026-04-13_combinations.md`) "
        "qui avait révélé que le cluster `E1/E5/D5/D6` (P18 trio avec `dd_cap=ON` "
        "et max_leverage ∈ [3, 14]) dominait le Sharpe WF sans perdre le CAGR cible. "
        "Phase 19 fouille densément cette région avec un grid `target_vol × max_leverage × dd_cap` "
        "et teste aussi 7 variations de poids autour du 80/10/10 baseline, plus 9 points "
        "expérimentaux à haut levier (ml ∈ [18, 24]).\n"
    )
    if p18_row:
        lines.append(
            f"Baseline de comparaison : **Phase 18 prod** — Sharpe WF "
            f"{p18_row['wf_avg_sharpe']:.3f}, CAGR {p18_row['annual_return']*100:.2f}%, "
            f"MaxDD {p18_row['max_drawdown']*100:.2f}%.\n"
        )
    if e5_row:
        lines.append(
            f"Référence top-5 Phase 18 : **E5** (ml=14 DDon) — Sharpe WF "
    ...
    for block_id, desc in (
        ...
        lines.append(
            f"- **{len(block_rows)} configs**. Meilleure : `{best['id']}` "
        ...
        if p18_row:
            d = best["wf_avg_sharpe"] - p18_row["wf_avg_sharpe"]
            lines.append(
                f"- vs Phase 18 prod : Δ Sharpe WF = {'+' if d >= 0 else ''}{d:.3f}, "
                f"Δ CAGR = {'+' if best['annual_return'] >= p18_row['annual_return'] else ''}{(best['annual_return'] - p18_row['annual_return'])*100:.2f}%."
            )
        lines.append("")
    ...
    if core_rows:
        ...
        for tv in tvs:
            ...
            for ml in mls:
                match = next(
                    (r for r in ddon if r["target_vol"] == tv and r["max_leverage"] == ml),
                    None,
                )
                if match:
...
def main() -> None:
    ...
    if args.smoke:
        output_root = Path("/tmp") / f"phase19_smoke_{report_date}"
        md_path = output_root / "phase19.md"
    else:
        output_root = _PROJECT_ROOT / "results" / f"phase19_{report_date}"
        md_path = _PROJECT_ROOT / "docs" / "research" / f"phase19_{report_date}_refined_leverage.md"
    output_root.mkdir(parents=True, exist_ok=True)
    md_path.parent.mkdir(parents=True, exist_ok=True)

    print("=" * 72)
    print("  Phase 19 — Refined leverage sweep")
    print("=" * 72)
    ...
    # Drop sleeves that are not in the P18 trio (we don't need them —
    # all Phase 19 configs use {MR_Macro, TS_Momentum_3p, RSI_Daily_4p}).
    sleeves_p19 = {k: sleeves[k] for k in _P18_SLEEVES}
    print(f"  Loaded {len(sleeves_p19)} sleeves: {sorted(sleeves_p19)}")
    print()

    grid = build_phase19_grid()
    if args.smoke:
        smoke_ids = {
            "P19c-tv28-ml12-dd_OFF",
            "P19c-tv28-ml14-ddON",
            "BL-P18prod",
            "BL-E5",
        }
    ...
    pf_all, metrics = native_parallel_sweep(grid, sleeves_p19)
    elapsed = time.perf_counter() - t0
    print(f"  Native sweep completed in {elapsed:.1f}s ({elapsed/max(len(grid),1):.2f}s per combo)")
    ...
    if not args.smoke and not args.no_bootstrap:
        ...
        for rank, row in enumerate(sorted_rows[:5], 1):
            ...
            t0 = time.perf_counter()
            try:
                stats = bootstrap_config(cfg, sleeves_p19, n_runs=args.bootstrap_runs)
                bootstrap_by_id[row["id"]] = stats
                el = time.perf_counter() - t0
                print(
                    f"  [top{rank}] {row['id']}: CAGR P5={stats['cagr_p05']*100:+.2f}% "
    ...
    json_data = {
        ...
        "n_configs": len(rows),
        "sleeves_loaded": sorted(sleeves_p19),
    ...
    # ── Export markdown ───────────────────────────────────────────
    md = build_phase19_markdown(rows, bootstrap_by_id, report_date)
    ...
    if not args.smoke:
        ...
        generate_top_n_artifacts(sorted_rows, grid, sleeves_p19, output_root)
    ...
    p18_rank_row = next((r for r in rows if r["id"] == "BL-P18prod"), None)
    if p18_rank_row:
        rank = next(i for i, r in enumerate(sorted_rows, 1) if r["id"] == "BL-P18prod")
        print(
            f"\n  Phase 18 prod (BL-P18prod) ranks #{rank} of {len(sorted_rows)}  "
            f"(sharpe={p18_rank_row['wf_avg_sharpe']:+.3f})"
        )
    print()

...
\hypersetup{
  pdftitle={Stratégie FX Tri-Signaux --- Guide pédagogique client},
...
FxMultiSleeve fonctionne, \emph{comment} chacun de ses trois moteurs prend ses
...
\item \textbf{Drawdown maximal} strictement inférieur à $35\,\%$~;
...
\begin{insightbox}
\textbf{Pourquoi trois moteurs plutôt qu'un seul ?} Parce que chaque régime de
...
\subsection{Trois moteurs, une seule philosophie}

Les trois moteurs partagent une racine commune : ils exploitent tous des \emph{anomalies micro-structurelles} bien documentées dans la littérature académique sur le marché des changes, plutôt que de s'appuyer sur des prédictions macroéconomiques discrétionnaires. Cette discipline rend le système \emph{auditable} et \emph{testable}, et limite la dérive comportementale.
...
Le portefeuille combine les trois moteurs avec une allocation statique et une couche commune de ciblage de volatilité~:
...
\midrule
\rowcolor{softBlue} Moteur 1 --- MR Macro & $80\,\%$ & M1 intraday & Mean reversion & 4 majors \\
...
Au-dessus de cette allocation, un \textbf{ciblage de volatilité global} ajuste quotidiennement le levier appliqué de façon à maintenir une volatilité réalisée du portefeuille proche de $75\,\%$ annualisé (cible élevée, justifiée par la diversification entre moteurs). Le levier est plafonné à $64\times$ et est recalculé chaque jour à 21h~UTC sur la base de la volatilité observée des 21 et 63 derniers jours.

\begin{warningbox}
\textbf{Les chiffres de levier (jusqu'à $64\times$) supposent un compte bancaire
US offshore ou un compte EU \og Professionnel \fg{} avec capital significatif.}
...
%  § 2 — ARCHITECTURE
% ═══════════════════════════════════════════════════════════════════════
\section{Architecture des trois moteurs}
...
Le Moteur~1 reçoit 80\,\% parce qu'il a la volatilité intrinsèque la plus basse
...
moteurs sont approximativement équipondérés.
\end{thesisbox}
...
les flux deviennent unidirectionnels en raison de la \og fuite vers la qualité \fg,
...
\item \textbf{Fenêtre de news} : l'instant courant ne doit pas être dans une fenêtre de $\pm 15$~minutes autour d'une publication économique \og high impact \fg{} américaine (NFP, CPI, FOMC, etc.).
\end{itemize}
...
La couche de risque s'applique uniformément aux trois moteurs et constitue la \emph{moitié invisible} de la stratégie~: elle ne génère pas de signal, mais elle modère, plafonne et protège.

\subsection{Ciblage de volatilité global}

Chaque jour à 21h~UTC, après la fermeture des positions intraday du Moteur~1, la couche de risque recalcule un \textbf{levier global} commun aux trois moteurs. La formule~:
...
\item $\text{max\_leverage} = 64$ (plafond dur).
\end{itemize}
...
À chaque seconde (\emph{throttling} 1~Hz), la couche de risque compare l'\emph{equity} courante au plus haut historique persistant. Si le \emph{drawdown} dépasse $20\,\%$, la stratégie ferme \textbf{immédiatement toutes les positions} de tous les moteurs et arme un verrou~: tant que le verrou n'est pas levé manuellement (input \code{Inp\_ResetDDState}), aucun nouveau trade n'est ouvert.

\subsection{Plafond d'utilisation de la marge}

Indépendamment du \emph{drawdown} en P\&L, la marge engagée auprès du broker est surveillée toutes les 30~secondes. Deux seuils~:
...
Cette protection est spécifique au Moteur~1 (intraday). Elle interroge le calendrier économique MetaTrader pour identifier les publications USD \og high impact \fg{} (NFP, CPI, FOMC, retail sales, GDP, \ldots) et bloque toute nouvelle entrée dans une fenêtre de $\pm 15$~minutes autour de chaque publication.
...
\item Pré-paiement via une réduction proportionnelle du \emph{sizing} (\texttt{slip\_drag}) pour couvrir les sorties \og non-déclenchées \fg{} (time-stop, fermeture quotidienne, inversion de signal).
\end{enumerate}
...
Lors des publications \og high impact \fg{} (NFP, CPI, FOMC), le \emph{spread} EUR/USD typique passe de $1$ pip à $8$--$10$ pips pendant 1 à 5 minutes. Cette élargissement transitoire peut détruire un trade quasi-instantanément.
...
\midrule
\code{Inp\_AllocMRMacro}    & $0{,}80$  & Jamais (sauf rééquilibrage stratégique majeur). \\
\code{Inp\_AllocTSMomentum} & $0{,}10$  & Jamais. \\
\code{Inp\_AllocRSIDaily}   & $0{,}10$  & Jamais. \\
\code{Inp\_EnableDDCap}     & \texttt{true} & Garder activé en production. À désactiver uniquement pour tests d'optimisation. \\
\code{Inp\_DDCap}           & $0{,}20$  & À ajuster selon la tolérance au risque du compte. $0{,}15$ pour un compte conservateur, $0{,}25$ pour un compte plus tolérant. \\
\code{Inp\_EnableMarginCap} & \texttt{true} & Garder activé. \\
\code{Inp\_MarginCapPct}    & $0{,}50$  & À adapter selon la marge dont vous disposez en réserve. \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Paramètre} & \textbf{Défaut} & \textbf{Quand le toucher ?} \\
\midrule
\code{Inp\_GlobalTargetVol}    & $0{,}75$ & Cible volatilité du portefeuille. À réduire à $0{,}50$ ou $0{,}30$ sur compte EU retail $30\times$. \\
\code{Inp\_GlobalMaxLeverage}  & $64{,}0$ & Plafond levier. À ajuster au levier maximum offert par votre broker. \\
...
\midrule
Sharpe ratio (rf=0) & $> 1{,}0$ acceptable, $> 1{,}5$ excellent & Rendement annualisé divisé par volatilité annualisée. Mesure l'efficacité d'utilisation du risque. Notre baseline backtest~: $1{,}33$. \\
CAGR & $\in [10\,\%,\,15\,\%]$ & Mandat client. En dehors de cet intervalle, à auditer. \\
MaxDD equity & $< 35\,\%$ & Mandat client. Le DD-cap interne $20\,\%$ donne déjà un \emph{buffer}. \\
...
Sur la fenêtre validée, $6$ années sur $7$ sont positives, ce qui est attendu pour un Sharpe combiné de $1{,}33$. Une année négative isolée est une variance normale~; deux années consécutives négatives mériteraient une analyse de régime.
...
\item Un sentiment de marché contraire (\og je sens que l'EUR va monter \fg).
\end{itemize}
...
Trade individuel \og bizarre \fg{} ?                    & Continuer & Continuer \\
\bottomrule
\end{tabular}
\end{center}
...
\item[\textbf{Sharpe ratio}] Ratio rendement excédentaire / volatilité, indicateur d'efficacité d'utilisation du risque. Calculé ici avec un taux sans risque nul (\og rf=0 \fg).
...
R : Oui, comme toute stratégie quantitative. Le \emph{drawdown maximum} historique observé sur 5{,}4~ans est de $9{,}3\,\%$, mais des creux plus profonds restent statistiquement possibles. Le mandat client autorise jusqu'à $35\,\%$.

\paragraph{Q : Pourquoi 80\,\% sur le Moteur~1 et seulement 10\,\% sur les autres ?}
R : Cette répartition ne reflète pas la part du P\&L mais la \emph{contribution au risque}. Le Moteur~1 a la volatilité la plus basse, donc il peut porter une allocation nominale plus grande sans dominer le risque global. Voir \S~\ref{sec:architecture}.

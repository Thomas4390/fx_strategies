...
\paragraph{Exemple numérique Apogee.} $N = 6$~essais (les six composantes candidates entrées en validation croisée), $\mathrm{Var}[\widehat{\mathrm{SR}}] \approx 0.089$, ce qui donne $\mathbb{E}[\max \mathrm{SR}\mid N = 6] = 0.273$. Le Sharpe observé $\widehat{\mathrm{SR}} = 1.31$ dépasse ce seuil d'un facteur $4.8$, d'où DSR~$= 1.000$. Ce $N$ ne compte que les candidates de la validation croisée, pas les configurations explorées en amont~: c'est la limite du test.
...
Pour le portefeuille Apogee, le MinBTL affiché est $2.10$~ans pour $\mathrm{SR} = 1.31$ avec $N = 6$~essais. On dispose de 7.5~ans, soit plus de trois fois le minimum statistique requis.

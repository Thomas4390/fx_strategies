...
\input{tables/x10_robustness.tex}
...
$0{,}35$. Le critère passe.

\begin{warningbox}
\textbf{Attention à ce que ce chiffre dit, et à ce qu'il ne dit pas.} Le
\code{PBO} passe son seuil~: le classement relatif des configurations ne
présente pas le niveau de retournement attendu d'une sélection fortement
surajustée. Cela ne crée pas un signal rentable. Les vingt-sept configurations
restent négatives et aucune ne passe le plateau~; le \code{PBO} répond à une
question de classement, pas à celle de l'espérance absolue.
\end{warningbox}
...
\begin{itemize}
  \item \XTenCriteriaPassing{} critères sur \XTenCriteriaCount{} sont
  franchis~: le nombre de transactions et le \code{PBO} ;
  \item \XTenCriteriaFailing{} sont en échec, dont
  \XTenCriteriaFailingInterpretable{} avec une mesure interprétable ;
  \item seul le critère OOS 2026 est en \emph{non-mesure}. Le critère
  MetaTrader~5 est mesuré et échoue parce que son espérance est négative.
\end{itemize}
...
interprétable » $\Rightarrow$ \textbf{ne pas déployer}. Il y en a
\XTenCriteriaFailingInterpretable. La

...
\fancyfoot[R]{\footnotesize\color{slateGrey}\textsf{Guide pédagogique client --- Juillet 2026}}
...
à l'intention d'un comité d'investissement, et (iii)~le \emph{rapport
...
\lettrine[lines=2, findent=2pt, nindent=0pt]{F}{x}MultiSleeve est une stratégie quantitative diversifiée sur le marché des changes (FX) déployée en un seul \emph{Expert Advisor} sur MetaTrader~5. Elle combine \textbf{quatre moteurs de rendement indépendants}~--- trois sur les devises, un sur trois instruments directionnels (or, dollar-yen, argent)~--- chacun spécialisé dans un régime de marché différent, regroupés sous une couche commune de gestion du risque.
...
\begin{itemize}
\item \textbf{Rendement annualisé (CAGR)} de l'ordre de $40\,\%$~--- la cible a été relevée par le mandant, les versions antérieures visaient $[10\,\%,\,15\,\%]$~;
...
Les trois moteurs FX partagent une racine commune : ils exploitent tous des \emph{anomalies micro-structurelles} bien documentées dans la littérature académique sur le marché des changes, plutôt que de s'appuyer sur des prédictions macroéconomiques discrétionnaires. Le quatrième, sur l'or, le dollar-yen et l'argent, relève de la même discipline mais d'une littérature différente~--- le \emph{time series momentum}. Cette discipline rend le système \emph{auditable} et \emph{testable}, et limite la dérive comportementale.
...
\item Le \sleeve{Moteur~4 (Gold Momentum)} capture le \emph{time series momentum} de l'or, du dollar-yen et de l'argent sur des horizons de plusieurs semaines. C'est le seul moteur à sortir des devises, et la source de rendement dominante du portefeuille~--- $87\,\%$ du résultat mesuré (cf.~\ref{sec:gold}).
\end{itemize}
...
\midrule
\rowcolor{softBlue} Moteur 1 --- MR Macro & $67\,\%$ & M1 intraday & Mean reversion & 4 majors \\
\rowcolor{softYellow} Moteur 2 --- TS Momentum & $9\,\%$ & D1 swing & Trend following & 3 majors \\
\rowcolor{softGreen} Moteur 3 --- RSI Daily & $9\,\%$ & D1 swing & Mean reversion & 3 majors \\
\rowcolor{lightGrey} Moteur 4 --- Gold Momentum & $15\,\%$ & D1 swing & Trend following & 3 actifs \\
\bottomrule
\end{tabular}
\end{center}
...
\toprule
 & \cellcolor{softBlue}\textbf{Moteur 1} & \cellcolor{softYellow}\textbf{Moteur 2} & \cellcolor{softGreen}\textbf{Moteur 3} & \cellcolor{lightGrey}\textbf{Moteur 4} \\
 & \cellcolor{softBlue}MR Macro & \cellcolor{softYellow}TS Momentum & \cellcolor{softGreen}RSI Daily & \cellcolor{lightGrey}Gold Momentum \\
\midrule
Allocation & 67\,\% & 9\,\% & 9\,\% & 15\,\% \\
Timeframe & M1 (1 minute) & D1 (1 jour) & D1 (1 jour) & D1 (1 jour) \\
Type & Mean reversion intraday & Suivi de tendance & Mean reversion swing & Suivi de tendance \\
Indicateurs & VWAP + Bollinger 5\,$\sigma$ & EMA 20/50 + RSI(7) & RSI(14) & Ensemble de signes 40/60/120/250 \\
...
Trades / an & $\sim 57$ & $\sim 90$ & $\sim 6$ & $\sim 17$ \\
Win rate & $58\,\%$ & $63\,\%$ & $67\,\%$ & $42\,\%$ \\
Part du P\&L mesuré & $3{,}1\,\%$ & $7{,}3\,\%$ & $2{,}6\,\%$ & $\mathbf{87{,}0\,\%}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent\footnotesize Chiffres mesurés sur le backtest d'exécution MetaTrader~5 de
référence (2021-01-01 $\rightarrow$ 2026-04-30, $905$~transactions). La dernière ligne est
la part du profit net réalisée par chaque moteur~--- à ne pas confondre avec l'allocation,
qui dimensionne une exposition et ne prédit pas un résultat.\normalsize

\subsection{Pourquoi cette répartition de poids ?}

\begin{thesisbox}
\textbf{Le poids reflète le rapport rendement/volatilité, pas la part du P\&L.}
Le Moteur~1 reçoit 67\,\% parce qu'il a la volatilité intrinsèque la plus basse
...
\smallskip
Le Moteur~4 échappe à cette logique~: ses $15\,\%$ ne sont pas un poids
vol-équipondéré mais un plafond d'exposition délibéré. Sa volatilité propre
($17{,}8\,\%$ annualisée en mesure standalone) est plusieurs fois celle des moteurs
FX, si bien qu'à $15\,\%$ du capital il porte déjà une part majeure du budget de risque
\emph{et} l'essentiel du rendement du portefeuille. Il n'en pesait que $10\,\%$ avant
juillet~2026~: la hausse est un choix assumé, discuté en~\ref{sec:gold}.
\end{thesisbox}
...
\caption{Courbe d'équité log-scale du portefeuille combiné FxMultiSleeve sur 2020--2026, avec décomposition par moteur. Les courbes ne se déplacent pas en synchrone~--- c'est précisément ce que permet la diversification.}
\end{figure}

\begin{insightbox}
\textbf{La courbe combinée est plus régulière que chacune des composantes prise
séparément.} Lorsque l'un des moteurs traverse un creux, les autres compensent
...
% ═══════════════════════════════════════════════════════════════════════
\section{Moteur 4 --- Gold Momentum (suivi de tendance sur trois instruments)}
\label{sec:gold}

\subsection{Intuition économique}

Les trois premiers moteurs travaillent sur des paires de devises. Le quatrième
travaille sur trois instruments~--- l'or (\code{XAUUSD}), le dollar-yen
(\code{USDJPY}) et l'argent (\code{XAGUSD})~--- et exploite une régularité
différente~: le
...
\smallskip
Depuis juillet~2026, le même signal est appliqué à deux autres instruments, avec
un tiers du budget chacun. Le \textbf{dollar-yen} apporte une tendance longue de
nature différente (le différentiel de taux) et, contrairement à l'or, un
\emph{portage positif}~: les frais de nuit y sont une recette et non une charge.
L'\textbf{argent}, médiocre pris isolément, casse la dépendance du résultat à la
seule trajectoire de l'or~--- c'est le poste qui divise le repli de la sleeve, de
$-76{,}7\,\%$ pour l'or seul à $-38{,}0\,\%$ pour le trio en mesure isolée. En
contrepartie, c'est celui dont l'historique de données de courtier propres est le
plus court.
...
\begin{insightbox}
\textbf{Le moteur est \emph{long-only}.} Il n'y a pas de vente à découvert
(\code{Inp\_Gold\_AllowShort = false}). Quand l'ensemble est négatif sur un
instrument, le moteur est simplement hors marché sur cet instrument. Ce choix supprime la queue de risque la plus
...
référence de cinq ans et quatre mois, le moteur n'a ouvert que \textbf{93~positions}
tous instruments confondus (35 sur l'or, 35 sur le dollar-yen, 23 sur l'argent).
C'est le moteur le moins actif du portefeuille rapporté à son poids, et c'est
cohérent avec un signal d'horizon long.
...
\textbf{Paramètre} & \textbf{Valeur} & \textbf{Pourquoi cette valeur ?} \\
\midrule
\code{Inp\_Gold\_Symbols} & \texttt{XAUUSD,\allowbreak USDJPY,\allowbreak XAGUSD} & Les symboles suivis par le moteur, séparés par des virgules. Le budget est réparti à parts égales entre eux. Le suffixe de compte (\code{.c}) est ajouté automatiquement. \\
...
\code{Inp\_Gold\_TargetVol} & $0{,}55$ & Volatilité annualisée visée pour ce moteur pris isolément. Élevée par construction~: c'est un moteur de rendement, borné en aval par son allocation de $15\,\%$, elle-même divisée en trois budgets égaux. \\
\code{Inp\_Gold\_MaxLeverage} & $6{,}6$ & Plafond de levier propre au moteur, qui borne l'exposition quand la volatilité réalisée d'un instrument s'effondre. \\
...
\code{Inp\_Gold\_SlippageBps} & $2$ & Slippage modélisé, en points de base par côté. Plus bas que sur les paires FX~: les marchés traités ici sont profonds aux volumes en jeu. \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{Équité du Moteur~4 pris isolément, à son dimensionnement propre et \emph{avant} l'allocation de $15\,\%$. La progression est très irrégulière~: de longues périodes hors marché ou sans progrès, ponctuées de quelques mouvements de grande amplitude qui font l'essentiel du résultat.}
\end{figure}

Pris isolément, non pondéré et non leviérié, le moteur rend $727{,}5\,\%$ sur la
période complète mesurée (2000--2026), soit un CAGR de $8{,}50\,\%$, pour une
volatilité annualisée de $17{,}78\,\%$ et un Sharpe de $0{,}48$. Son repli maximal
isolé atteint $-40{,}47\,\%$~--- l'or seul y montait à près du double.

\begin{warningbox}
\textbf{Ce moteur porte le résultat, et il porte le risque.} Sur le backtest
d'exécution MetaTrader~5 de référence, il produit $87{,}0\,\%$ du profit net du
portefeuille avec $15\,\%$ de l'allocation et $10\,\%$ des transactions. Son taux
de réussite est de $41{,}6\,\%$~: trois positions sur cinq perdent de l'argent, et
l'ensemble du résultat vient d'une minorité de mouvements longs. C'est mieux que
les $31{,}4\,\%$ de l'or seul~--- trois instruments produisent des séquences de
tendance moins synchrones~--- mais la nature du moteur est inchangée.
...
\item \textbf{Une longue série de pertes est le régime normal}, pas un signal de
      dysfonctionnement. Un moteur de suivi de tendance à $40\,\%$ de réussite
      passe l'essentiel de son temps à perdre de petites sommes.
\item \textbf{La performance du portefeuille est concentrée, et elle l'est plus
      qu'avant.} La hausse du poids a fait passer la part de ce moteur dans le
      résultat de $80\,\%$ à $87\,\%$. Si les trois instruments entrent ensemble
      dans un régime sans tendance durable, le portefeuille perd sa principale
      source de rendement et retombe sur les trois moteurs FX, dont les CAGR isolés
      s'échelonnent entre $0{,}5\,\%$ et $1{,}7\,\%$ en mesure standalone~--- un tout
      autre ordre de grandeur.
\item \textbf{Élargir la sleeve n'a pas diversifié le portefeuille}, seulement la
      sleeve. Les trois instruments ont tendu dans le même sens sur toute la
      fenêtre mesurée~; rien ne garantit qu'ils divergeront quand il le faudrait.
\item \textbf{Le repli isolé de $-40{,}47\,\%$ est réel}, même s'il est ramené à
      l'échelle du portefeuille par l'allocation de $15\,\%$ et par la couche de
      ciblage de volatilité globale.
\end{itemize}
\end{warningbox}

\begin{insightbox}
\textbf{Pourquoi $15\,\%$~?} Le poids de ce moteur a été porté de $10$ à $20\,\%$
en juillet~2026, après l'élargissement de la sleeve à trois instruments~: la mesure
donnait un profil risque-ajusté meilleur, au prix d'un repli plus profond et d'une
concentration accrue. \textbf{Le 28~juillet, il a été ramené à $15\,\%$}~--- non
parce que la mesure s'était dégradée, mais parce qu'un test de sur-ajustement, qui
n'avait jamais été appliqué à cette grille d'allocations, a été passé et qu'il
\emph{échoue} à ce poids. Sa lecture~: dans près de deux découpes temporelles sur
trois, l'allocation la mieux classée sur une moitié des données retombe sous la
médiane sur l'autre. La règle de décision, écrite et horodatée avant le calcul,
imposait alors de reculer d'un cran dans la grille.

\smallskip
À $15\,\%$, le backtest d'exécution donne un Sharpe de $1{,}01$ et un repli
d'équité de $47{,}0\,\%$, contre $0{,}89$ et $44{,}3\,\%$ pour la configuration
précédente à $10\,\%$ sur l'or seul. Il faut lire ces chiffres pour ce qu'ils
sont~: le Sharpe continuait de monter jusqu'au bord de la grille testée, ce qui est
la signature d'une période favorable à ce moteur plutôt que d'un maximum. C'est
précisément pourquoi le poids retenu n'est pas celui que la mesure désignait.
Aller au-delà serait une décision à prendre sur des données futures, pas sur celles
qui ont servi à la calibrer.
\end{insightbox}

\subsection{Pourquoi ce moteur n'utilise ni martingale ni grille}
\label{sec:gold_sizing_pedago}

La taille des positions de ce moteur ne dépend jamais de ce qui vient de se
passer. Après trois gains ou après trois pertes, la règle est la même. Ce choix
n'a rien d'évident~--- il est même contraire à ce que proposent la plupart des
robots vendus sur l'or~--- et il a été tranché par la mesure, pas par principe.
Cette sous-section explique ce qui a été testé et ce qui a été trouvé. L'étude a
été conduite sur l'or, instrument d'origine de la sleeve~; sa conclusion porte sur
le \emph{mécanisme} de dimensionnement, commun aux trois instruments.

\subsubsection*{Les trois règles progressives, en langage clair}

\begin{center}
\renewcommand{\arraystretch}{1.3}
\setlength{\tabcolsep}{5pt}
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{2.9cm}>{\raggedright\arraybackslash}p{5.2cm}>{\raggedright\arraybackslash}p{6.3cm}}
\toprule
\textbf{Règle} & \textbf{Ce qu'elle fait} & \textbf{Ce qu'on espère en obtenir} \\
\midrule
\textbf{Martingale} & Après une perte, la position suivante est plus grosse (ici $\times 1{,}5$ ou $\times 2$). & Récupérer d'un coup le terrain perdu quand le marché se retourne enfin. \\
\textbf{Anti-martingale} & L'inverse~: on augmente après un gain, on réduit après une perte. & Charger quand la tendance porte, s'effacer quand elle ne porte plus. \\
\textbf{Grille} & On rachète par paliers à mesure que le prix baisse contre la position. & Abaisser le prix de revient moyen pour repasser en profit plus vite au rebond. \\
\bottomrule
\end{tabular}
\end{center}

\begin{insightbox}
\textbf{Le point commun de ces trois règles, et il est essentiel~:} aucune ne
touche au signal. Elles ne changent ni le moment d'acheter, ni le moment de
vendre~--- seulement le \emph{montant}. Une règle de taille ne peut donc pas
rendre rentable un signal qui ne l'est pas. Elle ne fait que redistribuer dans le
temps le résultat que le signal produit déjà.
\end{insightbox}

\subsubsection*{Ce que chaque règle aurait réellement fait}

Les deux figures ci-après ne sont pas des schémas de principe~: chaque règle a été
appliquée, telle qu'elle est codée, à la suite réelle des $47$~positions prises par
le moteur sur l'or entre 2019 et 2026.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{gold_sizing_martingale.png}
\caption{\textbf{La martingale double la mise après chaque perte.} En haut, sur
tout l'historique~: la taille repart à $1\times$ après un gain, et grimpe à chaque
perte. En bas, le détail de la plus longue série de pertes réellement subie~---
sept d'affilée. À la huitième position, la règle réclame $128$~fois la mise de
...
\caption{\textbf{La grille rachète par paliers quand le prix descend.} En haut,
l'exposition atteinte position par position~: la grille ajoute au moins un palier
sur $20$~des $47$~positions, et atteint son plafond de $4\times$ sur sept d'entre
elles. En bas, la mécanique détaillée sur une position réelle~: trois rachats en
deux jours, exposition multipliée par quatre, prix de revient moyen abaissé de
\$$1\,974$ à \$$1\,953$. Ici le cours est remonté ensuite~--- c'est le scénario
qui rend la méthode attirante, et il est réel. Le problème est ce qui se passe
quand il ne remonte pas.}
\end{figure}

\subsubsection*{Ce que dit l'arithmétique avant même de tester}

\begin{examplebox}
\textbf{Une martingale suppose qu'une série de pertes s'arrête vite.} Sur
l'historique réel de ce moteur, la plus longue série de positions perdantes
consécutives a été de \textbf{sept}.

\smallskip
Avec un doublement à chaque perte et une première position de
$1\,000$~€, la huitième aurait dû engager $2^{7} = 128$~fois la mise
initiale, soit \textbf{128\,000~€} --- pour une stratégie à qui l'on avait
alloué $1\,000$~€ au départ. Le compte aurait été liquidé bien avant~--- non pas
parce que le signal avait tort, mais parce que la règle de taille exige un
capital qui n'existe pas.
\end{examplebox}

Une deuxième vérification, tout aussi simple, condamne la grille. Elle consiste à
regarder ce qui se passe \emph{après} que le prix se soit déplacé contre la
position. Si moyenner à la baisse était fondé, la suite devrait être favorable en
moyenne. C'est l'inverse qui est mesuré~: après une excursion adverse de $-0{,}5$
à $-1\,\%$, le rendement suivant est \textbf{négatif} ($-0{,}44$~point de base,
très significatif statistiquement). À l'inverse, quand la position est déjà en
profit de plus de $0{,}3\,\%$, la suite est nettement positive ($+2{,}48$~points
de base). \textbf{Sur cet actif, ajouter du capital quand ça va mal revient à en
ajouter là où l'espérance est la plus mauvaise.}

\subsubsection*{Comment les règles ont été comparées}

Trois précautions, sans lesquelles la comparaison n'aurait rien voulu dire.

\begin{enumerate}
\item \textbf{À risque égal.} Une martingale prend des positions plus grosses en
      moyenne~--- $125\,\%$ d'exposition contre $69\,\%$ pour la règle plate. Si
      on la compare telle quelle, elle \enquote{gagne} simplement parce qu'elle
      risque davantage. Chaque règle a donc été ramenée au même niveau de risque
      avant d'être jugée.
\item \textbf{Sur deux périodes distinctes.} Une période pour comparer
      (2019--2025), puis une période mise de côté dès le départ et jamais
      utilisée pendant la mise au point (2025--2026), qui contient le repli de
      $-28\,\%$ de l'or.
\item \textbf{En regardant les pertes extrêmes, pas seulement la moyenne.} Pour
      chaque règle, on estime la probabilité de perdre plus de la moitié du
      capital, et l'ampleur du repli dans les $5\,\%$ de scénarios les plus
      défavorables.
\end{enumerate}

\subsubsection*{Le résultat, et pourquoi il est décisif}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Règle} & \multicolumn{2}{c}{\textbf{Période 2019--2025}} & \multicolumn{2}{c}{\textbf{Période 2025--2026}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
 & Sharpe & rang & Sharpe & rang \\
\midrule
\rowcolor{softYellow} Anti-martingale & $0{,}81$ & \textbf{1\textsuperscript{er}} & $1{,}35$ & \textbf{6\textsuperscript{e}} \\
Plate (celle qui est livrée) & $0{,}60$ & 7\textsuperscript{e} & $1{,}42$ & 4\textsuperscript{e} \\
\rowcolor{softYellow} Martingale & $0{,}38$ & \textbf{dernière} & $1{,}54$ & \textbf{2\textsuperscript{e}} \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{\textbf{Le même signal passé dans les quatre règles, ramenées au même
niveau de risque.} À gauche la période de comparaison, à droite la période mise de
côté. L'anti-martingale (vert) domine la première et finit dernière sur la
seconde~; la martingale (rouge) fait exactement l'inverse. La règle plate (bleu
marine) reste au milieu dans les deux cas.}
\end{figure}

\begin{thesisbox}
\textbf{La règle la meilleure sur la première période devient l'une des pires sur
la seconde, et réciproquement.} Même code, mêmes signaux d'achat et de vente,
deux périodes qui se suivent~--- et un classement qui s'inverse.

\smallskip
C'est \emph{le} résultat de cette étude. Un classement qui s'inverse entre deux
périodes voisines ne mesure pas une qualité de la règle~: il mesure la suite de
gains et de pertes que chaque période contenait par hasard. Autrement dit, ces
règles ne sont pas reproductibles, et un bon résultat passé n'y annonce rien.
\end{thesisbox}

Il serait tentant de retenir la martingale sur la foi de la seconde période, où
elle finit deuxième. Deux raisons l'interdisent.

D'abord, cette période ne contient aucun épisode de stress~: elle donne une
probabilité de perte majeure nulle pour \emph{toutes} les règles testées, et un
écart de repli maximal de $0{,}7$~point entre la meilleure et la pire. Elle est
donc incapable de distinguer les règles sur le risque~--- précisément l'axe qui
disqualifie la martingale.

Ensuite, ce qui reste stable d'une période à l'autre n'est pas le classement,
c'est la \textbf{forme des pertes extrêmes}.
...
\begin{tabular}{lcc}
\toprule
 & \textbf{Martingale} & \textbf{Plate} \\
\midrule
Repli dans les $5\,\%$ de scénarios les pires (2019--2025) & $62{,}8\,\%$ & $\mathbf{54{,}8\,\%}$ \\
Repli dans les $5\,\%$ de scénarios les pires (2025--2026) & $45{,}3\,\%$ & $\mathbf{32{,}2\,\%}$ \\
Probabilité de perdre plus de la moitié du capital (2019--2025) & $1{,}40\,\%$ & $\mathbf{0{,}10\,\%}$ \\
\bottomrule
\end{tabular}
\end{center}

\begin{warningbox}
\textbf{Ce qu'une martingale fait réellement.} Elle ne fait pas gagner davantage
en moyenne. Elle échange \enquote{beaucoup de petits gains} contre \enquote{une
perte rare et très lourde}. C'est pourquoi elle affiche souvent d'excellents
résultats sur un historique~: la perte qui la caractérise ne s'est pas encore
produite. \textbf{Une martingale peut présenter une courbe impeccable jusqu'à la
transaction qui l'anéantit.}

\smallskip
C'est exactement ce que montre la seconde période de test, et c'est la raison
pour laquelle son bon classement n'a pas été retenu comme argument.
\end{warningbox}

\begin{insightbox}
\textbf{Conclusion~: le moteur est livré en taille plate.} Trois raisons, par
ordre d'importance~:
\begin{enumerate}
\item aucune règle progressive ne se comporte de la même façon d'une période à
      l'autre~;
\item la seule période qui les favorise est aveugle au risque qui les
      disqualifie~;
\item la taille plate n'a aucun réglage~--- ni multiplicateur, ni nombre de
      paliers, ni seuil. Elle ne peut donc pas être sur-ajustée, et elle se situe
      au milieu du classement dans les deux périodes plutôt que première dans
      l'une et dernière dans l'autre.
\end{enumerate}

\smallskip
Les trois règles restent présentes dans le code livré, testées et sécurisées,
mais désactivées. Le détail chiffré complet figure au rapport d'investissement.
\end{insightbox}
...
\item $\text{target\_vol} = 0{,}37$ (la cible),
...
\textbf{Il n'existe donc aucun coupe-circuit automatique.} Le repli d'équité constaté sur le backtest de référence atteint $47{,}0\,\%$, et rien ne l'aurait interrompu s'il était allé plus loin. C'est un choix assumé du mandant, pas un oubli.
...
\item Cette valeur est conservatrice~: certaines positions en \emph{carry positif} génèrent un swap positif, d'autres en \emph{carry négatif} coûtent davantage. La modélisation absolue à $0{,}5$ bps absorbe le pire cas.
\item Le Moteur~1 est intraday et ne paie jamais de swap. Les Moteurs~2 et~3 paient sur leurs détentions multi-jours. \textbf{Le Moteur~4 est de très loin le premier payeur}~: ses positions se comptent en semaines, et l'or comme l'argent ne rapportent rien à qui les détient.
\item \textbf{Le dollar-yen fait exception, et c'est une des raisons de sa présence}~: le portage y est \emph{positif}. Sur le backtest de référence, il rapporte $+3\,391$~USD de swap au Moteur~4 pendant que l'or lui en coûte $-10\,818$ et l'argent $-7\,314$.
\end{itemize}
...
Swap rollover cumulé    & $\sim -14\,950$ USD \\
\quad dont Moteur~4 (Gold Momentum) & $-14\,740$ USD \\
\quad dont Moteurs~2 et~3           & $-1\,038$ USD \\
...
\textbf{Le \emph{swap} est concentré sur le Moteur~4}, qui détient des positions
pendant des semaines~: $-10\,818$~USD sur l'or et $-7\,314$~USD sur l'argent, contre
$+3\,391$~USD \emph{gagnés} sur le dollar-yen dont le portage est positif. Les
Moteurs~2 et~3 paient $-1\,038$~USD à eux deux~; le Moteur~1 (intraday) le ramène à
zéro. Rapporté au profit net de \$68\,534, ce coût de portage représente environ un
cinquième du résultat brut~--- il est déjà déduit de tous les chiffres publiés, mais
il faut savoir que le rendement de ce moteur se gagne \emph{malgré} lui.
...
\midrule
\code{Inp\_AllocMRMacro}      & $0{,}67$  & Jamais (sauf rééquilibrage stratégique majeur). \\
\code{Inp\_AllocTSMomentum}   & $0{,}09$  & Jamais. \\
\code{Inp\_AllocRSIDaily}     & $0{,}09$  & Jamais. \\
\code{Inp\_AllocGoldMomentum} & $0{,}15$  & Jamais. La somme des cinq allocations doit valoir exactement $1{,}0$. \\
\code{Inp\_AllocH1Momentum}   & $0{,}0$   & Jamais. Ce cinquième moteur est compilé dans le programme mais non alloué~: il a été testé et écarté (cf.~\ref{sec:params}). Le laisser à zéro. \\
...
\code{Inp\_Gold\_SlippageBps}      & $2$  & Idem, sur les trois instruments du Moteur~4. \\
...
\code{Inp\_MagicGold}    & $835$  & Idem Moteur~4, sur ses trois instruments. \\
...
\midrule
Sharpe ratio (rf=0) & $> 1{,}0$ acceptable, $> 1{,}5$ excellent & Rendement annualisé divisé par volatilité annualisée. Mesure l'efficacité d'utilisation du risque. Notre baseline backtest~: $1{,}01$. \\
CAGR & $\approx 40\,\%$ visé & Mandat client. Baseline backtest~: $39{,}65\,\%$. \\
MaxDD equity & \emph{mesuré} & Aucun plafond contractuel. Repli constaté au backtest~: $47{,}0\,\%$. \\
...
Total trades & $\sim 900$ sur 5{,}3~ans & Ordre de grandeur attendu, en fonction de la fréquence des moteurs. \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
MR Macro       & $304$ & $57{,}9\,\%$ & $\$47{,}93$   & $-\$54{,}07$ \\
TS Momentum    & $479$ & $62{,}8\,\%$ & $\$67{,}91$   & $-\$94{,}53$ \\
RSI Daily      & $33$  & $66{,}7\,\%$ & $\$82{,}62$   & $-\$50{,}58$ \\
Gold Momentum  & $89$  & $41{,}6\,\%$ & $\$1\,626{,}94$ & $-\$335{,}42$ \\
\bottomrule
\end{tabular}
\end{center}
...
trades) pour $3{,}1\,\%$ du P\&L seulement, le Moteur~3 a peu de trades mais un win
rate élevé ($67\,\%$), le Moteur~2 a beaucoup de trades pour un edge marginal mais
joue son rôle de diversification. \textbf{Le Moteur~4 est l'inverse de tous les
autres}~: il perd trois fois sur cinq, mais son gain moyen est vingt-quatre fois celui
du Moteur~2 --- c'est la signature d'un suivi de tendance, et c'est ce qui explique
qu'il porte $87\,\%$ du résultat en $10\,\%$ des transactions. Une dégradation du
win rate du Moteur~3 sous $60\,\%$ serait un signal d'alerte~; sur le Moteur~4,
c'est l'absence de \emph{gros} gagnant sur plusieurs trimestres qui alerterait, pas
le win rate.
\end{insightbox}
...
R : Oui, et il faut s'attendre à des creux importants. Le \emph{drawdown} d'équité maximal observé sur le backtest de référence est de $47{,}0\,\%$~--- il était de $44{,}3\,\%$ avant que la sleeve momentum ne passe de $10$ à $15\,\%$ du capital~--- et les simulations de résistance placent une trajectoire sur vingt au-delà de $70\,\%$. Le mandat ne fixe \textbf{aucun plafond}~: le dimensionnement a été calibré pour atteindre la cible de rendement, et il multiplie le risque dans la même proportion.

\paragraph{Q : Pourquoi 67\,\% sur le Moteur~1 et si peu sur les autres ?}
...
Il faut d'ailleurs bien distinguer les deux~: sur le backtest de référence, le Moteur~4 ne pèse que $15\,\%$ de l'allocation mais produit $87\,\%$ du résultat, tandis que le Moteur~1 en produit $3{,}1\,\%$. Un poids d'allocation dimensionne une exposition, il ne prédit pas un profit.
...
R : Elle reste opérationnelle. Le rendement absolu sera plus faible parce que la cible de volatilité de $0{,}37$ ne pourra pas être atteinte avec un levier plafonné à $30\times$. On peut alternativement réduire la cible à $0{,}30$ pour un profil plus modéré et compatible.
...
\begin{itemize}
\item \textbf{Guide d'installation client} (\texttt{guide\_installation/ApogeeInvest\_Strategie1\_GuideInstallation.pdf})~: procédure pas-à-pas pour déployer la stratégie sur MetaTrader~5 Windows en moins de 30~minutes.
\item \textbf{Rapport exécutif} (\texttt{rapport\_technique/ApogeeInvest\_Strategie1\_FXMultiMoteurs\_SyntheseExecutive.pdf})~: synthèse destinée aux comités d'investissement.
\item \textbf{Rapport quantitatif complet} (\texttt{rapport\_technique/ApogeeInvest\_Strategie1\_FXMultiMoteurs\_RapportTechnique.pdf})~: documentation exhaustive de la méthodologie de validation, incluant les tests \emph{walk-forward}, \emph{bootstrap}, \emph{deflated Sharpe}, \emph{probability of backtest overfitting}, \emph{Hansen SPA}.
...
\begin{center}
{\interspaced\color{apogeeGold}\footnotesize APOGÉE INVEST \enskip\textbullet\enskip GUIDE PÉDAGOGIQUE CLIENT \enskip\textbullet\enskip JUILLET 2026\par}

% ═══════════════════════════════════════════════════════════════════════
%  ANNEXE NON INCLUSE dans le rapport technique — conservée comme référence méthodologique.
%  Constaté orphelin par l'audit du 2026-07-26 : aucune référence dans tout le
%  dépôt, donc absente de tous les PDF livrés. Ses chiffres ont été remis à
%  jour le 2026-07-26 pour qu'elle reste incluable telle quelle ; si vous
%  l'incluez, revérifiez-les contre tables/robustness_*.tex.
% ═══════════════════════════════════════════════════════════════════════
...
\end{equation}
Sur les 8.5~ans de données journalières de ce portefeuille ($T = 2\,146$), cette formule donne un écart-type annualisé de $0.34$ sur l'estimateur~: un Sharpe vrai nul peut produire un Sharpe observé de cet ordre par pure chance. Le bruit d'estimation est loin d'être négligeable, et c'est ce qui explique la largeur de l'intervalle de confiance publié pour le Sharpe (tableau~\ref{tab:robustness_verdict_summary}).
...
\paragraph{Choix de $L$.} \textsc{Hall}, \textsc{Horowitz} et \textsc{Jing} (1995) recommandent $L \propto T^{1/3}$. Pour $T = 2\,146$~barres journalières, cette règle donne $L \approx 13$. Notre choix de $L = 20$ est légèrement conservateur, ce qui donne des intervalles de confiance légèrement plus larges et donc plus prudents.
...
\paragraph{Exemple numérique Apogee.} $N = 382$~essais (les configurations distinctes du registre d'essais du projet), $\widehat{\sigma}[\widehat{\mathrm{SR}}] = 0.062$ entre les allocations rivales, ce qui donne $\mathbb{E}[\max \mathrm{SR}\mid N = 382] = 0.185$. Le Sharpe observé $\widehat{\mathrm{SR}} = 0.99$ dépasse ce seuil d'un facteur $5$, d'où DSR~$= 1.000$.

\paragraph{La limite de ce test, et où elle se voit.} Le seuil dépend beaucoup moins de $N$ (il croît en $\sqrt{2\ln N}$) que de la \emph{dispersion} des essais rivaux. Ici les $382$~essais sont des variantes d'un même portefeuille~: leur dispersion est faible, le seuil est bas, et le verdict favorable est en partie mécanique. Appliquée aux $21$~instruments du criblage, dont les Sharpe s'étalent de $-1.05$ à $+0.78$, la même formule donne $\mathbb{E}[\max \mathrm{SR}] = 2.89$~--- seuil qu'aucun des trois instruments retenus ne franchit. Le DSR valide l'assemblage, pas la sélection des composants.
...
\end{equation}
qui représente la fraction de l'\emph{edge} qui survit à la correction. Un ratio supérieur à $80\,\%$ est un bon signe~--- l'\emph{edge} est grand relativement à la variance d'estimation et les corrections multi-test ont peu d'emprise. Pour le portefeuille Apogee, le ratio BHY est $96.11\,\%$ (tableau~\ref{tab:robustness_overfitting}).
...
Pour le portefeuille Apogee, le MinBTL affiché est $12.25$~ans pour $\mathrm{SR} = 0.99$ avec $N = 382$~essais. On dispose de $8.5$~ans~: \textbf{le minimum statistique n'est pas atteint}, et il ne le sera que par accumulation de temps réel, pas par un traitement supplémentaire des mêmes données.
...
Pour Apogee, PBO~$= 0.649$~: dans $64{,}9\,\%$ des partitions, l'allocation la mieux classée in-sample retombe sous la médiane hors échantillon. C'est \emph{au-dessus} du seuil critique de $0.5$~--- ce test échoue, et c'est lui qui a fait ramener le poids de la sleeve momentum de $20\,\%$ à $15\,\%$ (section~\ref{sec:advanced_robustness}). Le PBO est mesuré sur la grille d'allocations réellement parcourue~; les éditions antérieures de ce rapport le calculaient sur la matrice des quatre \emph{sleeves} et annonçaient $0.305$, une valeur qui répondait à une question jamais posée dans ce projet.
...
Les tests implémentés dans \texttt{framework.robustness} ne sont pas tous appliqués au même stade du \emph{workflow}. Voici la hiérarchie recommandée~:

...
\item \textbf{Alerte automatique de drawdown live}~: arrêt automatique du trading si le drawdown depuis le \emph{high water mark} dépasse 15\,\%. Le seuil à 15\,\% est choisi pour déclencher l'alerte \emph{avant} d'atteindre la bande la plus favorable du bootstrap ($-34.93\,\%$ au 95\textsuperscript{e}~percentile du repli), en laissant une marge de décision manuelle.

\item \textbf{Monitoring par moteur, et par instrument sur le Moteur~4}~: calcul du Sharpe glissant sur 63~jours pour chaque moteur individuel, avec alerte si un moteur passe sous 0.5. Le Moteur~4 portant $87{,}0\,\%$ du résultat, sa contribution est suivie instrument par instrument (or, dollar-yen, argent). Cela permet de détecter une dégradation structurelle avant que le portefeuille combiné n'en souffre visiblement.
...
\item \textbf{Validation du slippage réel}~: pendant le premier mois de paper-trade, mesurer l'execution spread réel sur le Moteur~1 (hypothèse du backtest~: 15~pb), sur les Moteurs~2 et~3 (hypothèse~: 10~pb) et sur le Moteur~4 (hypothèse~: 2~pb par côté, sur les trois instruments). Si le slippage réel dépasse l'hypothèse, recalibrer la baseline de performance.

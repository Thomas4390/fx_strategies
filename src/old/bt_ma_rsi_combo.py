...
def _run_marsi_sim(close_arr, ma_window, rsi_window, entry_th, fees, slippage):
    ...
    ts = close_arr.shape
    col = close_arr[:, 0]
    fast_ma = vbt.generic.nb.ewm_mean_1d_nb(
        col, span=ma_window, minp=ma_window, adjust=False
    )
    slow_ma = vbt.generic.nb.ewm_mean_1d_nb(
        col, span=ma_window * 3, minp=ma_window * 3, adjust=False
    )
...
def marsi_batch(close_arr, ma_ws, rsi_ws, entry_ths, fees, slippage):
    ...
    for idx in prange(total):
        ...
        i2 = (idx // n3) % n2
        i3 = idx % n3
        results[idx] = _run_marsi_sim(
            close_arr, ma_ws[i1], rsi_ws[i2], entry_ths[i3], fees, slippage
        )
    return results

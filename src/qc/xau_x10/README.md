...
4. `create_backtest` / `read_backtest`. Les ordres sont récupérables par API
   (`read_backtest_orders`) : leur `tag` porte
   `scenario|level|ts_decision|stop|target|r_est`, ce qui en fait le **canal
   principal** de réconciliation.
...
5. **Spread réel contre spread constant.** La référence facture la médiane
   horaire de `costs_xau_intraday.yml` (repli 0,29 $) ; QC lit le bid/ask réel
   de chaque `QuoteBar`. `R` (§8) et les lots (§11) sont donc calculés sur des
   demi-fourchettes légèrement différentes, ce qui peut faire basculer un
   candidat à la frontière `R = 1` ou un lot à la frontière du pas de 0,01.
...
| 2 | `atr,v,m,a,vwap,ema50_h1,dxy` | warm-up (écart 1), amorçage ATR, `ignore_na` de l'EMA DXY |
